    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        document.addEventListener('click', function(e) {
            if (e.target && e.target.classList.contains('detail-btn')) {
                ...
                var data = koordinatMap[koordinatId]; // Ambil data lengkap dari map

                if (data) {
                    var modalBody = document.getElementById('detail-content');

                    // 1. Buat bagian HTML yang umum untuk semua sumber data
                    let commonHtml = `
            ...
            `;
                    ...
                    // 2. Buat variabel untuk menampung HTML spesifik secara dinamis
                    let specificHtml = '';

                    // 3. Lakukan looping pada array 'keterangan_tambahan' yang dikirim dari controller
                    if (data.keterangan_tambahan && data.keterangan_tambahan.length > 0) {
                        data.keterangan_tambahan.forEach(function(item) {
                            // item akan berisi: { jdl_keterangan: "Labelnya", isi_keterangan: "Nilainya" }
                            specificHtml += `<tr><th>${item.jdl_keterangan}</th><td>${item.isi_keterangan || 'N/A'}</td></tr>`;
                        });
                    }
                    // **[MODIFIKASI SELESAI]**

                    // 4. Gabungkan HTML umum dan spesifik, lalu tampilkan di modal
                    modalBody.innerHTML = `
